...
const VOICE_ANALYSIS_SERVICE_URL = process.env.VOICE_ANALYSIS_SERVICE_URL; // Fetch from .env

if (!VOICE_ANALYSIS_SERVICE_URL) {
    console.error("VOICE_ANALYSIS_SERVICE_URL is not defined in the environment variables.");
    process.exit(1); // Exit if the variable is missing
}

router.get("/", async(req, res) => {
    const response = await axios.get(`${VOICE_ANALYSIS_SERVICE_URL}/`);
    res.send(response.data);
});


// Route to handle the POST request
router.post("/analyse-audio/", async (req, res) => {
    try {
      ...
      const { question_id, interview_id, user_id } = req.body;
  
      if (!question_id || !interview_id || !user_id) {
      ...
      const response = await axios.post(
        `${VOICE_ANALYSIS_SERVICE_URL}/analyse-audio/`,
        form,
        {
          headers: {
